// FinanceDashboardContent.js (or define within your page file)
"use client";
...
export default function FinanceDashboardContent() {
    ...
    useEffect(() => {
        if (!eventId) return; // skip if no valid event_id
    ...
    const handleEditExpense = (exp) => {
        ...
        // Map category name from DB back to categoryId using categories array
    ...
    return (
        ...
        <div className="min-h-screen bg-gray-50 pt-20 p-6 text-black">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="bg-white p-4 rounded-lg shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button onClick={() => setBudgetDetailsModalOpen(true)} className="text-primary text-sm font-medium hover:underline">
                            View Details
                        </button>
                    </div>
            ...
            {/* Modal Components for Budget Details and Add/Edit Expense */}
            ...
            <Modal isOpen={isExpenseModalOpen}
                   onClose={() => {
                       setExpenseModalOpen(false);
                       setEditingExpenseId(null);
                       setNewExpense({ categoryId: "", title: "", amount: "", description: "" });
                ...
                <form onSubmit={handleExpenseSubmit} className="space-y-4 text-black">
                    ...
                    <input type="text" placeholder="Title" value={newExpense.title}
                           onChange={(e) => setNewExpense({ ...newExpense, title: e.target.value })}
                           className="w-full border border-gray-300 rounded-lg px-4 py-2" required />
                    <input type="number" placeholder="Amount" value={newExpense.amount}
                           onChange={(e) => setNewExpense({ ...newExpense, amount: e.target.value })}
                           className="w-full border border-gray-300 rounded-lg px-4 py-2" required />
                    <textarea placeholder="Description" value={newExpense.description}
