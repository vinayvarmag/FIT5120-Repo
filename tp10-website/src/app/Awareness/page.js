...
export default function AwarenessPage() {
    ...
    const styleHeat = useCallback((feature) => {
        ...
        const [h, s, l] = hexToHSL(colours.current[selectedValue] || '#D1C4E9');
        const max = Math.max(
            ...Object.values(rawCounts.current).map((m) => m[selectedValue] || 0),
            1
        );
    ...
    const onEach = useCallback(
        (feature, layer) => {
            ...
            layer.on({
                ...
                mouseout: (e) =>
                    e.target.setStyle(showHeatmap ? styleHeat(feature) : styleClass(feature)),
            });
        },
        [showHeatmap, selectedValue, styleHeat, styleClass]
    );

    const gradientColors = useMemo(() => {
        if (!selectedValue) return { light: '#fff', dark: '#000' };
        const [h, s] = hexToHSL(colours.current[selectedValue] || '#D1C4E9');
        return {
    ...
    const maxCount = useMemo(() => {
        return selectedValue
            ? Math.max(...Object.values(rawCounts.current).map((m) => m[selectedValue] || 0), 1)
            : 0;
    }, [selectedValue]);
                ...
                {mode === 'Map' && (
                    <div className="flex gap-4 mt-2 h-[60vh]">
                        <div className="flex-3 rounded-2xl shadow-xl bg-white/90 backdrop-blur-lg ring-1 ring-black/10 overflow-visible">
                            <LocationMap
                                key={`${dataset}-${showHeatmap}-${selectedValue}`}
                                center={[-37.81, 144.96]}
                                zoom={7}
                        ...
                        <div className="flex-1 bg-white p-6 rounded-2xl shadow-xl ring-1 ring-black/10 overflow-auto">
                            {showHeatmap && selectedValue ? (
                                <>
                                    <h3 className="font-semibold mb-2">{selectedValue}</h3>
                                    <div
