...
export default function PrecinctPage() {
    ...
    // Reset heatmap on Overview view
    useEffect(() => {
        if (mode === "Overview") {
    ...
    const styleHeat = useCallback(
        (feature) => {
            ...
            const maxVal = Math.max(
                ...Object.values(rawCounts.current).map((m) => m[selectedVal] || 0),
                1
            );
            ...
            <div className="mx-auto max-w-screen-xl px-4">
                        ...
                        {["Map", "Overview"].map((m) => (
                            <button
                                ...
                                className={`px-5 py-2 text-sm font-medium rounded-full transition ${
                                    mode === m ? "bg-purple-800 text-white" : "text-black hover:bg-gray-300"
                                }`}
                            >
                                {m}
                            </button>
                ...
                {mode === "Map" && (
                    <div className="flex items-center mb-4">
                        ...
                        {/* Value selector appears only when Heatmap is on */}
                        {showHeatmap && (
                            <select
                                value={selectedVal}
                    ...
                    {mode === "Overview"
                        ? dataset === "Birthplace"
                            ...
                            : dataset === "Language"
                                ...
                                : "Top 10 Countries by Religion"
                        : showHeatmap
                            ? dataset === "Birthplace"
                                ? "Heatmap of Birthplace Distribution"
                                : dataset === "Language"
                                    ? "Heatmap of Language Distribution"
                                    : "Heatmap of Religious Affiliation"
                            : dataset === "Birthplace"
                                ? "Map of Top Countries by Birthplace"
                                : dataset === "Language"
                                    ? "Map of Top Languages Spoken"
                                    : "Map of Top Countries by Religion"}
                </div>
                ...
                {mode === "Overview" ? (
                            ...
                            datasets: [
                                {
                                    ...
                                    backgroundColor: overview.map((o) =>
                                        o.value ? "#6b21a8" : "transparent"
                                    ),
                                },
                            ],
                        }}
                    ...
                    <div className="flex gap-4 mt-2 h-[60vh]">
                        <div className="flex-1 rounded-2xl shadow bg-white/90 backdrop-blur-lg ring-1 ring-black/10 overflow-hidden">
                            <LocationMap
                                key={`${dataset}-${showHeatmap}-${selectedVal}-${JSON.stringify(
                                    countData?.rows
                                )}`}
                                center={[-37.81, 144.96]}
                                zoom={7}
